// Intro skit: Steve fails to build a house alone, Claude shows up,
// awkward silence, Steve asks for help, Claude does it effortlessly.
...
const uglyEvents = (delay = 0) => {
  ...
  return [
    ...
    { t: delay + 1.1,  x: ox + 4, y: oy,     z: oz + 3, block: 7 },
    ...
    { t: delay + 2.6,  x: ox + 2, y: oy,     z: oz,     block: 3 },
...
export const SKIT_SHOTS = [
  // 1. Wide on Steve alone, struggling to build his house
  {
    id: 'skit-establish',
    duration: 3.0,
    fadeIn: 0.6,
    camera: { type: 'dolly',
      from: [PLAYER_POS[0] - 6, PLAYER_POS[1] + 9, PLAYER_POS[2] + 14],
      to:   [PLAYER_POS[0] - 3, PLAYER_POS[1] + 5, PLAYER_POS[2] + 9],
      lookFrom: lookAtUgly, lookTo: lookAtUgly },
    overlay: { html: 'A long time ago in a voxel world…', t0: 0.4, t1: 2.6, fadeIn: 0.4, fadeOut: 0.6 },
    avatars: {
      Steve: { pos: PLAYER_POS, look: lookAtUgly, expression: 'focused' },
    },
    events: uglyEvents(0.2).slice(0, 6), // first half of the ugly placements happen here
  },
  // 2. Closer orbit on Steve continuing to make a mess
  {
    id: 'skit-building',
    duration: 4.0,
    camera: { type: 'orbit',
      center: [SKIT_X + 0.5, SKIT_Y + 1.5, SKIT_Z + 0.5], radius: 6.5, height: 3,
      startAngle: 1.2, endAngle: 2.2 },
    avatars: {
      Steve: { pos: PLAYER_POS, look: lookAtUgly, expression: 'focused', showTag: false },
    },
    events: uglyEvents(-2.0).slice(6), // remaining placements (shifted in time)
  },
  // 3. Slow circle around the ugly result with Steve dejected
  {
    id: 'skit-result',
    duration: 3.0,
    camera: { type: 'orbit',
      center: [UGLY_AT[0] + 0.5, UGLY_AT[1] + 1.5, UGLY_AT[2] + 0.5], radius: 7, height: 4,
      startAngle: 2.0, endAngle: 3.6 },
    avatars: {
      Steve: { pos: PLAYER_POS, look: lookAtUgly, expression: 'sad', showTag: false },
    },
  },
  // 4. Despair close-up — face the camera, sad
  {
    id: 'skit-despair',
    duration: 1.8,
    camera: { type: 'still',
  ...
  // 5. Pan reveals Claude has appeared. Steve looks up surprised.
  {
    id: 'skit-claude-appears',
    duration: 2.6,
    camera: { type: 'dolly',
    ...
    avatars: {
      Steve:  { pos: PLAYER_POS, look: lookAtClaude, expression: 'surprised' },
  ...
  // 7. Awkward silence: long beat, both characters static, no dialog
  {
    ...
    avatars: {
      Steve:  { pos: PLAYER_POS, look: lookAtClaude, expression: 'thinking', showTag: false },
      Claude: { pos: CLAUDE_POS, look: lookAtPlayer, expression: 'happy',    showTag: false },
    },
  },
  // 8. Steve asks
  {
